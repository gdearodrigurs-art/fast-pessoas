import { esquemaDefinicaoLotacao } from "@/dominios/colaboradores/esquemas";
import {
  definirLotacao,
  obterLotacoes,
...
export async function GET(
  ...
  try {
    ...
    const resultado = await obterLotacoes(idNumero);
...
export async function POST(
  ...
  try {
    ...
    const analise = esquemaDefinicaoLotacao.safeParse(corpo);
    if (!analise.success) {
    ...
    await definirLotacao(sessao, idNumero, analise.data);
    const resultado = await obterLotacoes(idNumero);

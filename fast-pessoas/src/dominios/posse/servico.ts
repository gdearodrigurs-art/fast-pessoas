...
export async function registrarPosse(
  ...
  if (dados.termo_documento_id) {
    ...
    // Costura 1.4×1.5: documento do CICLO de ciência não vira termo de posse.
    // A ciência do ciclo tem regras próprias (ler até o fim, recusa, ato,
    // versão vigente) e a ciência da posse as atropelaria — barra na origem.
    if (termo.exige_ciencia) {
      throw new ErroHttpCampo(
        400,
        "Este documento participa do ciclo de ciência e não pode ser usado como termo de posse — publique o termo fora do ciclo.",
        "termo_documento_id"
      );
    }
...
export async function darCienciaPosse(
  ...
  // Costura 1.4×1.5: a ciência de posse NÃO assina documento do ciclo — o
  // ciclo exige ler até o fim e tem recusa/ato/liberação; este atalho pularia
  // tudo isso. registrarPosse barra na origem; aqui é o cinto para dado antigo.
  if (termo.exige_ciencia) {
    throw new ErroHttp(
      409,
